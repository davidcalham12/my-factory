...
| `export_to_langfuse_storymaker.py` | StoryMaker `tools/` (`da1c8b6`) | la versión del examen: una sesión por novela, una traza por versión y **una traza por cambio** (generate, continue, reader_change, redo) con dos generaciones de coste **medido** (orquestador y agentes, del `modelUsage` de cada `result`); las llamadas por agente llevan sus cuatro cifras de uso y el coste estimado va a metadata (`estimated_cost_usd`), para no sumar estimado y medido; `--replace` borra las trazas del run y espera (endpoint v2 de observaciones) antes de reenviar, porque el SDK v4 no deja fijar ids de observación. Depende del backend de StoryMaker (`backend.commons.*`, `backend.costs.*`); no corre suelto | desde la raíz de StoryMaker: `python tools/export_to_langfuse.py output/<slug> [--dry-run] [--replace]` |
| `repetition.py` | StoryMaker `backend/linters/` (`b95abbf`), SPEC-EXAM-005 O1 | linter de prosa, solo stdlib y solo informe: frases reformuladas a ≤ 2 frases de distancia (Jaccard ≥ 0,6) y muletillas de tres palabras usadas ≥ 4 veces en un capítulo. Nació porque el juez vio una frase dicha dos veces en ch05 que los seis críticos dejaron pasar | `python tools/repetition.py ruta/ch*.md` |
