...
| `export_to_langfuse_storymaker.py` | StoryMaker `tools/` (`b95abbf`) | la versión del examen: una sesión por novela, una traza por versión, una generación por llamada a agente con **las cuatro cifras de uso** (input, output, cache creation, cache read) y el coste por partes **estimado**; `--replace` borra las trazas del run y espera (endpoint v2 de observaciones) antes de reenviar, porque el SDK v4 no deja fijar ids de observación. Depende del backend de StoryMaker (`backend.commons.*`); no corre suelto | desde la raíz de StoryMaker: `python tools/export_to_langfuse.py output/<slug> [--dry-run] [--replace]` |
